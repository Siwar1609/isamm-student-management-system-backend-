 ...
 import {pfeValidationSchema  }from '../../validations/pfeValidationSchema.js';
 ...
 // Ajouter un PFE
  export const addPFE = async (req, res) => {
    try{
        const {company_name,title,description,type,teacherId,studentId,numberOfStudents,affected,academicYear,documentId,periodId}=req.body;
      ...
      if (period.start_date > new Date()) {
        return res.status(400).json({ message: 'La période de dépôt n\'est pas encore ouverte.' });
      }
      ...
      return res.status(201).json({ message: 'PFE ajouté avec succès', pfe: savedPFE });
    } catch (err) {
      console.error(err);
      return res.status(500).json({ message: 'Erreur du serveur' });
    }
  };
  import { updatePFEValidation } from '../validations/pfeValidation';
...
export const updatePFE = async (req, res) => {
  try {
    ...
    if (currentDate > period.end_date) {
      return res.status(400).json({ message: 'Les délais de dépôt sont dépassés.' });
    }

    // Mise à jour du PFE avec les nouvelles informations
    const updatedPFE = await PFE.findByIdAndUpdate(id, updateData, { new: true });

    return res.status(200).json({ message: 'PFE mis à jour avec succès.', data: updatedPFE });
  } catch (err) {
    console.error(err);
    ...
    try {
      ...
      const studentDetails = await Promise.all(
        students.map(async (student) => {
          ...
          }).populate('teacherId').populate('documentId').populate('periodId').populate('academicYear'); 
  
          if (!pfeDetails) {
            return { student: student.name, message: 'Aucun PFE trouvé pour cet étudiant.' };
          }
          ...
          return {
            ...
            teacher: pfeDetails.teacherId ? pfeDetails.teacherId.name : 'Non affecté',
          };
        })
      );
      ...
      console.error(error);
      res.status(500).json({ message: 'Erreur lors de la récupération des informations PFE.', error });
    }
  };
...
export const choosePFE = async (req, res) => {
    ...
    try {
      ...
      if (pfe.teacherId) {
        return res.status(400).json({ message: 'Ce PFE a déjà été choisi par un autre enseignant.' });
      }
