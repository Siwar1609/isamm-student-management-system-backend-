...
export const fetchAcademicYear = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ error: e.message, message: 'Failed to fetch Academic Years' })
  }
}
export const fetchPendingAcademicYear = async (req, res) => {
  try {
    // Add status filter to only return pending academic years
    const academicyear = await AcademicYear.find({ status: 'pending' })

    if (academicyear.length === 0) {
      return res.status(404).json({
        message: 'No pending academic years found',
      })
    }

    res.status(200).json({
      model: academicyear,
      message: 'Pending Academic Years Fetched Successfully!',
    })
  } catch (e) {
    res.status(400).json({
      error: e.message,
