...
export const fetchStudentChoices = async (req, res) => {
  try {
    ...
    const studentChoices = await choicePFA.find({})
    ...
    const formattedChoices = studentChoices.map((choice) => ({
      ...
      approvalStatus: choice.approval ? 'Approved by Student' : 'Not Approved by Student',
      validationStatus: choice.validate ? 'Validated by Admin' : 'Not Validated by Admin',
    }));
...
export const autoAllocatePFA = async (req, res) => {
  try {
    ...
    const pfAs = await PFA.find({}).populate('list_of_student', 'name validated');
    ...
    for (const pfa of pfAs) {
      if (!pfa.affected && pfa.list_of_student.length > 0) {
        const approvedStudents = pfa.list_of_student.filter((student) => student.validated);
        ...
        if (approvedStudents.length === 1) {
          ...
          unassignedPFAs.push({
            title: pfa.title,
            conflictingStudents: approvedStudents.map((student) => student.name),
          });
        } else {
          // No approved students
          unassignedStudents.push(...pfa.list_of_student.map((student) => student.name));
        }
      }
    }
...
export const sendEmailToRecipients = async (req, res) => {
  ...
  try {
    ...
    if (type === 'first') {
      ...
      return res.status(400).json({ message: 'Invalid email type. Use "first" or "modified".' });
    }
